# 日本語入力 (fcitx5 + Mozc) — 仕組み・理由・ハマりどころ
...
- プロファイルの `DefaultIM` は起動時に使う IM。`keyboard-us` にして英数から始める (fcitx5 が `mozc` に書き戻すことがある。ハマりどころ参照)。
...
> テーマを変えても Tokyo Night のまま残るため置き換えた。旧一式は git の履歴にある
> (`assets/omarchy-tokyo-night/`)。
...
一式はこのリポジトリの [`files/`](files/) に保存
(`themed/` = テンプレート、`hooks/` = hook、`profile` と `classicui.conf` = 配る設定)。
