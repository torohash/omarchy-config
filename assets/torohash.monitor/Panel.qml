...
Panel {
  ...
  //   "scale"      - single slider row, selectedIndex = -1 sentinel like
  //                  brightness and text size. h/l steps one scale stop.
  //   "monitors"   - vertical display row list for enabling/disabling displays;
  //                  j/k walks each row.
  // Mouse hover on a target updates root state via the components' `hovered`
  // signal so keyboard cursor and pointer share one highlight.
  // Curated stops. Model.availableScales() drops any value Hyprland cannot
  // render on the focused display (scales that do not divide the mode into
  // whole logical pixels) and collapses duplicates, so the slider length
  // reflects what this panel can actually do. On 2880x1800 the survivors are
  // 1, 1.25, 1.5, 1.6, 1.67, 1.8, 1.88, 2, 2.5, 3, 4.
  readonly property var scalePresets: ["1", "1.25", "1.5", "1.6", "1.667", "1.8", "1.875", "2", "2.5", "3", "4"]
  ...
  // While a scale change is in flight the knob would otherwise snap back to the
  // old live value until the CLI round-trip lands, so the pending stop index
  // overrides it. -1 = follow the live monitor scale.
  ...
  function sectionCount(section) {
    ...
    if (section === "scale") return 0      // slider sentinel at -1
  ...
  function sectionFirstIndex(section) {
    if (section === "brightness" || section === "textsize" || section === "scale") return -1
    return 0
  }
  ...
  // h/l: step the scale slider one stop, mirroring adjustTextSize.
  function adjustScale(deltaSteps) {
    if (scaleValues.length === 0) return
    var next = currentScaleIndex() + deltaSteps
    if (next < 0) next = 0
    if (next > scaleValues.length - 1) next = scaleValues.length - 1
    scalePreviewIndex = next
  ...
  function clampCursor() {
    ...
    if (sectionIsSingleRow(focusSection)) {
      // brightness, text size, and scale are lone sliders (-1 sentinel).
      selectedIndex = -1
      return
    }
    if (count === 0) {
  ...
  function updateDisplays(displaysJson) {
    ...
    // The live scale caught up to the pending stop — hand the knob back to it.
    if (root.scalePreviewIndex >= 0 && root.activeScaleIndex() === root.scalePreviewIndex)
  ...
  // otherwise whatever the live monitor scale resolves to.
  ...
  onScaleValuesChanged: {
    if (root.scalePreviewIndex > root.scaleValues.length - 1) root.scalePreviewIndex = -1
    clampCursor()
  }
          ...
          Column {
            ...
            Item {
              ...
              Text {
                ...
                textFormat: Text.PlainText
                text: {
                  var raw = scaleSlider.dragging
                    ? root.effectiveScale(root.scaleValues[Math.round(scaleSlider.liveValue)])
                    : (root.scalePreviewIndex >= 0
                        ? root.effectiveScale(root.scaleValues[root.scalePreviewIndex])
                        : root.normalizeScale(root.monitorScale))
                  var label = raw === "" ? "—" : raw + "x"
                  // Name the monitor too when more than one display is in play.
                  if (root.focusedMonitor !== "" && root.enabledDisplayCount > 1)
                    return root.focusedMonitor + " · " + label
                  return label
                }
                color: Qt.darker(root.bar.foreground, 1.4)
                font.family: root.bar.fontFamily
            ...
            CursorSurface {
              ...
              PanelSlider {
                ...
                anchors.rightMargin: Style.space(6)
                minimum: 0
                maximum: Math.max(0, root.scaleValues.length - 1)
                step: 1
                integer: true
                tickCount: root.scaleValues.length
                value: root.currentScaleIndex()
                onReleased: function(v) {
                  root.scalePreviewIndex = Math.round(v)
                  root.setScale(root.scaleValues[Math.round(v)])
                }
              }
